...
PDFSIGN = (function() {
    ...
    function newSig(pdf, root, rootSuccessor, date, cert, password, reason) {
        ...
        var middle = '>\n/Type/Sig/SubFilter/adbe.pkcs7.detached/M(D:' + now(date) + '\')\n/ByteRange ';
        var byteRange = '[0000000000 0000000000 0000000000 0000000000]';
        var _reason = typeof reason !== 'undefined' ? reason : '';
        var end = '/Filter/Adobe.PPKLite/Reason(' + _reason + ')/ContactInfo()>>\nendobj\n\n';
        //all together
        ...
        pdf.xref.entries[annotEntry] = {
            offset: offsetAnnot,
            gen: 0,
            free: false
        };
        pdf.xref.entries[sigEntry] = {
            offset: offsetSig,
            gen: 0,
            free: false
        };
    ...
    function appendSig(pdf, root, rootSuccessor, date, cert, password, reason) {
        ...
        xrefEntries[contentRef.num] = {
            offset: startContent,
            gen: 0,
            free: false
        };
        xrefEntries[annotEntry] = {
            offset: startAnnot,
            gen: 0,
            free: false
        };
        xrefEntries[sigEntry] = {
            offset: startSig,
            gen: 0,
            free: false
        };
    ...
    var api = {
        signpdf: function(pdfRaw, cert, password, reason, date) {
            date = typeof date !== 'undefined' ? date : new Date();
            ...
            var rootSuccessor = findSuccessorEntry(pdf.xref.entries, root);
						// FIXME: reason が日本語に対応していない。
            if (!isSigInRoot(pdf)) {
                return newSig(pdf, root, rootSuccessor, date, cert, password, reason);
            } else {
                return appendSig(pdf, root, rootSuccessor, date, cert, password, reason);
            }
        }
    };
